...
class Networks(object):
    ...
    def num_filters(self, block_id, fmap_decay=1.0, fmap_max=4096):
        # filter_numbers = [256,256,128,128,128,128,64,64,32,32]
        # filter_numbers = [256,128,128,128,64,64,64,32,32]
        filter_numbers = [256,128,128,128,64,64,64,32,32] #if 9
        # filter_numbers = [256,128,128,128,64,64,64,32,32,32] #if 10
        # filter_numbers = [512,512,256,256,128,128,64,64,32,32] #if 10
        # fmap_base = self._n_filters
        return filter_numbers[block_id]
    ...
    def make_conditional_generator(self, latent_size):
        ...
        noise_size = self._end_size
        for i in range(1, self._num_res):
            noise.append(Cropping1D(cropping=(int(noise_size-int(noise_size/self._scale_base)-1),1))(noise[-1]))
            noise_size=int(noise_size/self._scale_base)
        ...
        for i in range(0, self._num_res-1):
            x = self.g_block(x, 
                             self.num_filters(i), 
                             self._kernel_len, 
    ...
    def make_conditional_discriminator(self):
        ...
        for i in range(self._num_res-1, -1, -1):
