...
# PROBLEM ATM
# =============================================================================
# addd pitch featrue , usinnng sample rate to pitch up and down
#  make it adjust the waveform plot right and prepare for saving have global sample rate
# parameter

# expand how far pcas can go, redo umap and ppca for 44.1khz

# Change which low,mid,high layers are assigned to
...
# def interp_w_layerwise(fix_layers=0):
#     zw_interp_fixed = []
#     for i in range(args.num_res-1):
#         zw_interp_fixed.append([])
#         for j in range(len(zw_interp)):
#             zw_interp_fixed[i].append(zw_interp[0]) 
    
#     zw_layers=[]
#     for i in range(fix_layers):
#         zw_layers.append(np.array(zw_interp_fixed[i]))

#     # w_layers=[]
#     for i in range(args.num_res-(fix_layers+1)):
#         zw_layers.append(zw_points_layers[i])
    
#     return sample(zw_layers, const_save, inc_n_save)



# def layerwise(w_mod, active_layers):
